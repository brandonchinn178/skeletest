...
-- | A predicate that matches any value matching the given function.
--
-- Equivalent to @f P.>>> P.eq True@, but with nicer error messages.
check :: forall a m. (Monad m) => (a -> Bool) -> Predicate m a
check =
  -- Will be replaced with checkImpl by plugin
  invariantViolation "P.check was not replaced"

-- | Implementation of 'check'.
-- Should not be used directly, generated by the plugin.
checkImpl :: forall a m. (Monad m) => String -> (a -> Bool) -> Predicate m a
checkImpl funcDisplay func =
  Predicate
    { predicateFunc = \actual -> do
        let success = func actual
        pure
          PredicateFuncResult
 ...
 where
  disp = "matches " <> Text.pack funcDisplay
  dispNeg = "does not match " <> Text.pack funcDisplay
